...
func OpenConnectionWithRedisClient(tag string, redisClient redis.Cmdable, errChan chan<- error) (Connection, error) {
	return OpenConnectionWithmqrRedisClient(tag, RedisWrapper{redisClient}, errChan)
}

func OpenConnectionWithmqrRedisClient(tag string, redisClient RedisClient, errChan chan<- error) (Connection, error) {
	...
	// log.Printf("mqr connection connected to %s %s:%s %d", name, network, address, db)
...
func (connection *redisConnection) StopAllConsuming() <-chan struct{} {
	...
	go func() {
		...
		close(finishedChan)
		// log.Printf("mqr connection stopped consuming %s", queue)
	}()
