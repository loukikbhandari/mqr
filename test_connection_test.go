...
func TestTestConnection(t *testing.T) {
	...
	assert.Equal(t, "mqr.TestConnection: delivery not found: things[0]", connection.GetDelivery("things", 0))

	queue, err := connection.OpenQueue("things")
	assert.NoError(t, err)
	assert.Equal(t, "mqr.TestConnection: delivery not found: things[-1]", connection.GetDelivery("things", -1))
	assert.Equal(t, "mqr.TestConnection: delivery not found: things[0]", connection.GetDelivery("things", 0))
	assert.Equal(t, "mqr.TestConnection: delivery not found: things[1]", connection.GetDelivery("things", 1))
	...
	assert.Equal(t, "mqr.TestConnection: delivery not found: things[1]", connection.GetDelivery("things", 1))
	assert.Equal(t, "mqr.TestConnection: delivery not found: things[2]", connection.GetDelivery("things", 2))
